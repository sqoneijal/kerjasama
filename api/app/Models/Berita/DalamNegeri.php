...
class DalamNegeri extends Common
{
   ...
   public function hapus(array $post): array
   {
      try {
         ...
         $table->delete();
         return ['status' => true, 'message' => 'Data berhasil dihapus.', 'post' => $post];
      } catch (\Exception $e) {
   ...
   private function handleSubmitTags($selectedTags, int $id_berita, string $user_modified): void
   {
   ...
   private function handleSubmitKategori($selectedKategori, int $id_berita, string $user_modified): void
   {
   ...
   public function getData(array $post)
   {
      ...
      $table->select('tb.id, tb.judul, tb.content, tkb.kategori, tkb.kategori, ttb.tags');
      $table->join('(select tkb.id_berita, json_agg(json_build_object(\'nama\', tmkb.nama, \'slug\', tmkb.slug)) as kategori from tb_kategori_berita tkb
      join tb_mst_kategori_berita tmkb on tmkb.id = tkb.id_kategori
      group by tkb.id_berita) tkb', 'tkb.id_berita = tb.id', 'left');
      $table->join('(select ttb.id_berita, json_agg(json_build_object(\'nama\', tmtb.nama, \'slug\', tmtb.slug)) as tags
